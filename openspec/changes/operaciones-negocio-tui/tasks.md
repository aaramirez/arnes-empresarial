...
**5.2** Mismo archivo, nace verde: `it` 3: `/logout` → texto 3 ⇒ `onSubmit` y la tool no se invoca (invariante 3); `it` 4: una confirmación marcada **antes** del `/logout` **no** confirma después de un nuevo `/login` (R8, L2/L3; escenario "Una confirmación pendiente no sobrevive a un re-login").
...
**6.2** Check de mutación M2. Quitar la llamada **L2** de `manejarLogout`. `npm test`: **debe fallar** U3-limpieza. Revertir y re-verificar. Evidencia en `docs/progreso/vX.Y-operaciones-negocio-tui/mutacion-limpieza-logout.md`.
...
- [ ] **7.1** `npm run typecheck` en verde.
- [ ] **7.2** `npm test` en verde (con `dist/` limpio); el test `:421` y el (iv) de `main.test.ts:682` pasan **sin editar**.
- [ ] **7.3** `npm run build` en verde.
