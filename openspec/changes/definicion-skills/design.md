...
4. **Costo asumido, medido y escrito: el proceso escanea el disco DOS veces** (una en el arranque, una en la memo del primer turno). Es un `readdirSync` sobre un puñado de directorios más un `readFileSync` por skill, **una sola vez por proceso**, contra un turno que cuesta segundos de latencia de modelo. Y las dos lecturas tienen responsabilidades distintas, no duplicadas: la del arranque es la **validación que aborta** (ADR 111), la de la memo es el **valor que viaja al turno**. Queda como **RD-49**.
...
| RD | Riesgo | Estado |
|---|---|---|
| **RD-49** | **Dos escaneos del disco por proceso** (arranque + memo del primer turno) | **Aceptado y medido** (ADR 115 pto 4). Compra la ausencia total de estado global mutable y de acoplamiento temporal. Disparador para revisarlo: que el cargador pase de decenas a miles de archivos |
...
## 13. Preguntas abiertas para el checkpoint humano

1. **ADR 114 — la propuesta tiene un hecho mal.** El ADR 104 pto 5 dice que ningún agente tiene `Read`; son **tres** (`planner`, `developer`, `reviewer`), y además esos roles **corren como agente principal**, así que `Options.skills` los alcanza. La conclusión de riesgo bajo se sostiene por otro motivo (delta de exposición cero), pero el argumento escrito hay que corregirlo — y de ahí sale el límite de **tool cero** que decidió RD-47. Si preferís que la corrección viva en una enmienda al ADR 104 en vez de en un ADR nuevo, se mueve.
2. **ADR 110 pto 2 — la whitelist de dos campos es la decisión más opinada del diseño.** Hace que un `SKILL.md` con `allowed-tools` **rompa el arranque**. Es lo que convierte el límite 2 del ADR 106 en un guard mecánico, y es más estricto que el SDK a propósito. Si te parece demasiado rígido para un repo que va a acumular skills, la alternativa es whitelist de dos + *warning* logueado para el resto — pero entonces el límite 2 vuelve a ser sólo una regla escrita.
3. **ADR 112 — la skill elegida es deliberadamente modesta.** `citar-conocimiento` no le agrega ninguna capacidad al arnés: mejora el **cómo** de una que ya tiene. Es lo que la hace segura (tool cero, sin plata, rollback trivial) y también lo que la hace poco espectacular. La candidata rica (`rastreo-de-impacto`, para el Developer) queda esperando a `AgentDefinition.skills`. Si el trabajo de grado necesita una demo más vistosa, esa conversación es sobre el ADR 107 pto 2, no sobre este change.
4. **ADR 115 pto 4 — dos escaneos de disco por proceso.** Es el precio explícito de no tener un setter global. Barato hoy; si preferís un solo escaneo, la contrapartida es estado mutable compartido entre el arranque y el turno.
