...
> **Progreso (sdd-apply, 5.1 + 5.3, 2026-09-24)**: **5.1** hecha, **sin commitear** (rama `hito/v3.20-enmascarar-password-en-tui`; el humano commitea). Las cinco mutaciones (M1-M5) se probaron una a la vez sobre `App.tsx`/`comando-empleado.ts`, con respaldo por copia (nunca `git checkout`/`stash`) y restauración verificada por `sha256sum` + `git diff -- src` vacío tras cada una. M1 (`TurnRecord.prompt` real) ⇒ T3 rojo. M2 (push siempre al historial) ⇒ T5 rojo. M3 (`PromptInput` sin máscara) ⇒ T8 y T1 rojos. M4: se eligió el off-by-one en el id (`comando-empleado.ts:682`, se quitó el `+ 1` final) ⇒ los cuatro casos literales de U1 rojos; hallazgo adicional: U4 (paridad con `parsearComando`) **no** detecta este off-by-one porque verifica un superconjunto, no la posición exacta del corte — documentado en `mutaciones.md` para un refuerzo futuro de U4. M5 (guarda del historial con el predicado rechazado `enmascararSecreto(prompt) !== prompt`, invertido a `===`) ⇒ T5b rojo. Evidencia completa (rojo + verde por mutación) en `docs/progreso/v3.20-enmascarar-password-en-tui/mutaciones.md`. Tras restaurar la última mutación: `git diff -- src` vacío, `npm run typecheck` verde, `npm test` con `dist/` limpio → 3307/3312 (162 archivos, 5 skips preexistentes; un flake conocido y preexistente de `src/main.test.ts` ajeno a este change, confirmado no reproducible en aislamiento). **5.3** hecha, **sin commitear**: se agregó una nota de reemplazo (2 líneas) junto a `openspec/changes/comandos-administracion-empleados/design.md:256`, re-`Grep`eada antes de editar; `git diff` de ese archivo = 2 líneas agregadas, 0 borradas. **5.2 sigue pendiente** (verificación manual, la ejecuta el humano); se dejó el esqueleto en `docs/progreso/v3.20-enmascarar-password-en-tui/README.md`, marcado PENDIENTE, sin fabricar resultados. Faltan Phase 0 (condicionada a I1, ya resuelta por el checkpoint según la nota de progreso de la tarea 0.1), 5.2, 6.x y el Reviewer.
...
- [x] **7.1** `docs/ARC42_Harness_Empresarial.md`: agregar el **ADR 300** (texto de `design.md` §2, copiado tal cual) y reescribir el Riesgo 5 (R1), `:792-800` (título, descripción, mitigación *"recomendada"*, condición de disparo *"cumplida por ADR 300"*). Reverificar el techo de ADR/RD (R8, I4).
...
Commit: `docs(arc42): registra el ADR 300 y reescribe el Riesgo 5 tras el enmascarado de la clave en la TUI (Hito 3.20, tarea 7.1)`

> **Progreso (sdd-apply, 7.1, 2026-09-24)**: hecha, **sin commitear** (rama `hito/v3.20-enmascarar-password-en-tui`; el humano commitea). Techo de ADR/RD reverificado antes de escribir: `rg "ADR 30[0-9]|RD-17[0-9]"` sobre todo el repo no encontró `ADR 300`/`RD-172` reclamados fuera de los propios artefactos de este change (el hermano `reembolso-resta-monto-vendido-en-reporte` sólo *menciona* el 300 como reservado por este change y usa 301/RD-173 para sí mismo — sin choque). Se agregó `## ADR 300: La TUI enmascara el secreto de los comandos \`secreto: true\` (v3.20, \`enmascarar-password-en-tui\`)` inmediatamente después de `## ADR 2` (única sección con ADRs con encabezado propio en el arc42; ADR 174 y vecinos sólo se citan inline en Concepto 8 y no tienen encabezado propio, así que "orden ascendente" se resuelve como "after the last headed ADR") y antes de `# Requerimientos de Calidad`, con el texto de `design.md` §2 preservado casi literal (se retiró sólo la nota de proceso *"no se edita el arc42 en esta fase"*, ya no aplica, y las referencias `[CP ...]`/números de tarea internos del design). Riesgo 5 (R1) reescrito completo (título, descripción, mitigación, condición de disparo) conservando la traza histórica *"(v3.7 `comandos-administracion-empleados` → cerrada parcialmente en v3.20 por ADR 300)"* y enumerando los tres residuales vigentes (R9, R2, R10) en vez del residual viejo. Se re-`Grep`earon `:285` y `:578` (ADR 174 inline): ninguno de los dos afirma directamente "visible en el transcripto" como residual — ambos sólo remiten a "Riesgo 5 (R1)" para el detalle — así que **no se tocaron** (siguen siendo ciertos tras la reescritura de Riesgo 5). `rg "ADR 300" docs/ARC42_Harness_Empresarial.md` → 6 coincidencias; `rg "visible en el transcripto" docs/ARC42_Harness_Empresarial.md` → sólo queda en menciones históricas/explícitamente negadas (nunca como residual aceptado vigente). `git diff --stat` = sólo `docs/ARC42_Harness_Empresarial.md` (+46/-5). **7.2 queda sin marcar**: es el checklist de cierre del humano.
