...
**Nota (`comandos-administracion-empleados`, PR1, ADR 185)**: `/estado-bot-prs` se agrega a `DESCRIPTORES` (queda en **dieciséis**), `privilegiado: true`, **`requiereAdministrador: false`** — es de solo lectura y no gatea por rol. Responde si el listener de webhooks está escuchando (puerto/path) y si `GITHUB_TOKEN` está configurado, ambos sin revelar valores — enmienda del checkpoint al diferido del ADR 178 de esta misma propuesta (configuración del bot de PRs desde la TUI sigue diferida; esto es sólo un "¿está andando?"). No escribe en `registro_acciones_empleado`: mismo criterio de comando de lectura pura. Como corrección asociada, la rama genérica de `parsearComando` para la forma `"sin_argumentos"` — que hasta acá tenía un único consumidor (`/logout`) y devolvía su tipo hardcodeado — pasa a devolver el tipo del descriptor encontrado, ahora que un segundo comando comparte esa forma. `/crear-empleado` y `/asignar-rol` (el resto de esta propuesta) llegan en una PR posterior, bloqueada hasta que `autorizacion-empleado` esté disponible en la rama — con ellos, `DESCRIPTORES` queda en dieciocho.
...
- **Baja de los tres comandos TUI de autoservicio** (`/devolucion`, `/solicitar`, `/cancelar-solicitud`, ADR 148 pto 2): se resuelven ahora por conversación vía esta misma herramienta. Los cinco comandos administrativos/HITL (`/aprobar-*`, `/rechazar-*`, `/reabrir-*`) no se tocan — el ADR 151 sigue **BLOQUEADO** por decisión del checkpoint, heredado tal cual de v3.2/v3.5 (R10, ya cerrado — ver Riesgo 3).
