...
**W3b** — integracion `it 7` (lineas 447-479), RD-170/D4: `/login ana` -> turno 1 (eco de confirmacion, BD identica, cero auditoria) -> `/ayuda` (`agentLabel: "sistema"`, la tool no corre, `onSubmit` no corre, BD identica) -> turno 2 "sí, confirmo" -> `aprobada` con una fila de auditoria de `ana`.
...
### 9.3 - referencias de linea en los docs de mutacion y remediacion: VERIFICADO

- `mutacion-limpieza-logout.md` cita la asercion de M2 en `src/build-on-comando-empleado.test.ts:3040`: **es correcta** (el hallazgo no se sostiene para ese punto). La linea 3040 es `expect(banco.operaciones.orden).toEqual(["limpiarEmpleado:ana", ...])` dentro de `U3 (limpieza, L2)` (`:3028`). Sin cambios en ese archivo. Tambien verificadas y correctas: `build-on-comando-empleado.ts:851` (L2, en `mutacion-limpieza-logout.md`) y `:1505` (la guarda, en `mutacion-guarda-sesion.md`).
- **Referencias obsoletas SI encontradas, en este mismo archivo**: el `it` nuevo de `build-on-submit.test.ts` `252-277` -> `252-276` (277 es el cierre del `describe`); integracion `it 5` `359-396` -> `359-388`; `it 6` `398-445` -> `398-439`; `it 7` `447-478` -> `447-479`. Corregidas. El apply-progress (Batch 4) repite `252-277`; se deja como registro historico y la correccion queda anotada aqui.
